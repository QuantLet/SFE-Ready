...
theta = -((exp(-d * tau) * dnorm(d1) * S * sig)/(2 * sqrt(tau))) - 
        (r * K * exp(-r * tau) * pnorm(y))

# Plot
title      = bquote(expression(paste("Strike price is ", .(K), ", interest rate is ", 
                                     .(r), ", dividend rate is ", .(d), 
                                     ", annual volatility is ", .(sig))))
title_main = expression(paste("Theta as function of the time to maturity ", 
...
lab_z      = list("Theta", cex = 1.1)



wireframe(theta ~ tau * S, drape = TRUE, ticktype = "detailed", main = title_main,
          sub = title, scales = scales_l, xlab = lab_x, ylab = lab_y, zlab = lab_z)
