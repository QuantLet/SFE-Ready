...
for (i in 1:15) {
    assign(paste("dax.garch", i, sep = ""), garchFit(substitute(~garch(i, 1), 
           list(i = i, 0)), rdax, trace = FALSE))
...
dax.LLF  = c(dax.garch1@fit$llh, dax.garch2@fit$llh, dax.garch3@fit$llh, 
             dax.garch4@fit$llh, dax.garch5@fit$llh, dax.garch6@fit$llh, 
             dax.garch7@fit$llh, dax.garch8@fit$llh, dax.garch9@fit$llh, 
             dax.garch10@fit$llh, dax.garch11@fit$llh, dax.garch12@fit$llh, 
             dax.garch13@fit$llh, dax.garch14@fit$llh, dax.garch15@fit$llh) * 
             (-1)

ftse.LLF = c(ftse.garch1@fit$llh, ftse.garch2@fit$llh, ftse.garch3@fit$llh,
             ftse.garch4@fit$llh, ftse.garch5@fit$llh, ftse.garch6@fit$llh, 
             ftse.garch7@fit$llh, ftse.garch8@fit$llh, ftse.garch9@fit$llh, 
             ftse.garch10@fit$llh, ftse.garch11@fit$llh, ftse.garch12@fit$llh, 
             ftse.garch13@fit$llh, ftse.garch14@fit$llh, ftse.garch15@fit$llh) * 
             (-1)
...
axis(side = 2, at = seq(0, 0.05, 0.01), label = seq(0, 0.05, 0.01), lwd = 1,
     cex.axis = 1)
...
plot(ftse.arch1@sigma.t, main = "FTSE 100", col = "blue3", axes = FALSE, 
     frame = TRUE, type = "l", ylab = "", xlab = "Time, t", ylim = c(0, 0.035), 
     sub = title)
axis(side = 2, at = seq(0, 0.05, 0.01), label = seq(0, 0.05, 0.01), lwd = 1, 
     cex.axis = 1)
axis(side = 1, at = where.put, label = labels[where.put], lwd = 0.5, 
